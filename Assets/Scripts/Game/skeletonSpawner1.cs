...
public class skeletonSpawner1 : MonoBehaviour
{
    [Header("References")]
    ...
    private void Update()
    {
        if (!towerTarget)
        {
            Debug.LogWarning("EnemySpawner has no tower target assigned.");
            return;
        }
    ...
    private void SpawnOne()
    {
        ...
        {
            Debug.LogWarning("EnemySpawner could not find a valid spawn position.");
            return;
        }
        ...
        NinjaMover mover = enemy.GetComponent<NinjaMover>();

        if (mover == null)
            mover = enemy.GetComponentInChildren<NinjaMover>();

        if (mover != null)
        {
            mover.SetTarget(towerTarget);
            mover.SetSpeed(enemySpeed);
        }
        else
        {
            Debug.LogWarning("Spawned enemy does not have a NinjaMover component.");
        }
    }
